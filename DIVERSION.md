...
are visible in the right-hand screen alone — the persona shown as a technical code (§4), the
default volume slider (§8), the read-only sample rate (§9), and the blend hint in the
description (§6):
...
Grouped by kind, most consequential first. The **#** column is the section number below.

🐛 bug &nbsp;·&nbsp; ✨ feature &nbsp;·&nbsp; ⚖️ design call &nbsp;·&nbsp; 🔧 polish &nbsp;·&nbsp; 🧰 infrastructure

| Kind | # | Difference | Porting effort |
|---|---|---|---|
| **Bug** | 4 | [Store the voice code, not the display name](#-4-store-the-voice-code-not-the-display-name) | Moderate |
| **Bug** | 2 | [Raise `ConfigEntryAuthFailed` on HTTP 401](#-2-raise-configentryauthfailed-on-http-401) | Trivial |
| **Bug** | 1 | [Reuse Home Assistant's shared aiohttp session](#-1-reuse-home-assistants-shared-aiohttp-session) | Trivial |
| **Bug** | 3 | [Derive `unique_id` from the config entry](#-3-derive-unique_id-from-the-config-entry) | One line |
| **Bug** | 7 | [Default language follows the voice, not the filter](#-7-default-language-follows-the-voice-not-the-filter) | Small |
| **Bug** | 6 | [Treat blended voices as unclassifiable](#-6-treat-blended-voices-as-unclassifiable) | Trivial |
| **Feature** | 5 | [Classify voices the server reports but the table doesn't list](#-5-classify-voices-the-server-reports-but-the-table-doesnt-list) | Small |
| **Feature** | 8 | [Configurable default volume](#-8-configurable-default-volume) | Small |
| **Design** | 10 | [Reload on any entry change, not just options](#-10-reload-on-any-entry-change-not-just-options) | Trivial |
| **Design** | 9 | [Sample rate is read-only](#-9-sample-rate-is-read-only) | Small |
| **Polish** | 11 | [`integration_type` in the manifest](#-11-integration_type-in-the-manifest) | One line |
| **Infra** | 12 | [A test suite that runs without Home Assistant](#-12-a-test-suite-that-runs-without-home-assistant) | Self-contained |
| **Infra** | 13 | [CI and automatic releases](#-13-ci-and-automatic-releases) | Self-contained |

Items 1, 2, 3, 6, 10 and 11 are self-contained and apply cleanly against origin's current
...
**Bug** (in the fork's own earlier version of item 5 — origin never had this, but will inherit
it if it takes item 5).
...
## ⚖️ 10. Reload on any entry change, not just options
